...
namespace GreatBrawl
{
    public class ResourceManager
    {
            ...
            where TResult : class
        {
            ...
            var prefab = Resources.Load(type + "/" + name, typeof(GameObject)) as GameObject;
            var go = GameObject.Instantiate(prefab, position, new Quaternion(0f, 0f, 0f, 0f));
            var component = go.GetComponent(typeof(TResult)) as TResult;
            return component;
        }
        
        public static TResult CreatePrefabInstance<TEnum, TResult>(TEnum resource, Transform transform)
            where TEnum : struct, IComparable, IConvertible, IFormattable
            where TResult : class
        {
            var type = typeof(TEnum).Name;
